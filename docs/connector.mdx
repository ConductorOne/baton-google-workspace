...
| get_user_data_transfer | `transfer_id` (string, required) | Reads the provider transfer's actual owners, overall status, and per-application statuses. Never starts a transfer. |
| remove_user_alias | `user_id`, `alias` (strings, required) | Removes an editable alias from the selected user. |
| add_user_alias | stable `user_id`, `alias` (strings, required) | Adds an alias to the selected user. Same-user replay is idempotent; conflicts never move or remove an alias. |
...
User sync and targeted reads use native Google Directory user objects. Configured filter exclusions use gRPC `NotFound` with `ErrorInfo` reason `RESOURCE_FILTERED`; genuine provider permission errors and malformed successes remain errors.
...
Drive privacy inputs remain case-insensitive and default to both private and shared; the connector sends Google's uppercase `PRIVATE`/`SHARED` values. Calendar retain-resources uses the documented empty parameter set. Those inputs are fully matched only while safely adopting an in-progress mutating transfer; `get_user_data_transfer` only reports actual provider state.

Calendar Data Transfer covers future, non-private events in the source user's primary calendar with a guest or resource. It does not migrate all past, private, or secondary-calendar content.

Alias actions operate only on the selected user. They reject primary addresses, non-editable aliases, and targets outside the configured customer. The dedicated alias collection provides idempotency and readback; conflicts never move an alias from another account. A failed mutation or readback returns an error. An alias absent from the selected account is not a claim that its address is globally available.

### Requestable alias creation

`add_user_alias` and `remove_user_alias` use the existing Directory API user-provisioning scope. They require a concrete configured customer ID, validate the provider target against it, derive primary/noneditable checks from that target, and use only the selected user's dedicated alias collection for idempotency and readback. Add issues at most one insert; a conflict is successful only when selected-user readback proves attachment. Neither action moves, removes, or probes another owner.

<Warning>
A connector action does not grant self-service permission. Administrators configure any audience, target binding, alias/domain policy, and approval separately. This connector neither infers requester identity nor implements a C1 approval system.
</Warning>
