...
Targeted user reads and user sync pages expose `google_user_state` on the resource profile. It includes the provider user/customer identity and, when returned by Google, separate `suspended`, `archived`, `change_password_at_next_login`, `is_mailbox_setup`, `aliases`, and `non_editable_aliases` fields. `observed_at` records the read time. An omitted field is unknown, not false or an empty list. The legacy aggregate enabled/disabled status remains unchanged and is not a readiness predicate. Filtered user reads return an explicit error rather than successful absence.
