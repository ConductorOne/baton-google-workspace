...
Targeted group reads include `group_settings` and require the Groups Settings API and `apps.groups.settings` scope. Missing authorization, a failed settings request, or a response with no supported settings fails the group read. Returned settings contain only values supplied by Google; omitted individual fields remain unknown and are not defaulted. Normal group listing does not fetch settings for every row. `allow_web_posting` controls the web forum, not permission for external email senders.
...
Enable the Admin SDK API. Add the Cloud Identity API for stable enterprise application IDs. Targeted group reads and settings changes also require the Groups Settings API.
...
| Groups Settings API | `groupssettings.googleapis.com` | Required for targeted group reads and settings changes | Group `Get` and the `modify_group_settings` connector action |
...
<Step>
Repeat for the **Groups Settings API** to enable targeted group reads and settings changes.
</Step>
</Steps>
...
Google has no read-only Groups Settings scope. Both sets include `apps.groups.settings` because targeted group reads require it; this scope also permits editing group settings. Directory group-read permission alone does not grant access to settings.
...
| `apps.groups.settings` | Read group settings. Google provides no read-only scope for this API; this scope also permits editing settings |
...
| `apps.groups.settings` | Read and edit group settings. Requires the Groups Settings API |
| `cloud-identity.inboundsso.readonly` | Optional. Resolve SAML app IDs to stable identifiers. Without it, SAML app IDs fall back to display names |
</Tab>
</Tabs>
