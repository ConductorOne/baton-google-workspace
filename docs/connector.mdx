...
Both actions return two comma-separated lists: `updated_fields`, naming what changed, and `skipped_fields`, naming any provided field that was not applied and why. A response of `success: true` means only that the call completed. Check `skipped_fields` to see whether `manager_email` was skipped.
</Note>
...
stringData:
  ...
  BATON_CREDENTIALS_JSON: <Contents of your service account JSON key>
