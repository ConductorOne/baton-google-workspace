...
`add_user_alias` is a user-resource action using the existing Directory API user-provisioning scope. It requires a stable Google user ID rather than an email or alias selector, plus the expected current primary email and customer ID. Those submitted preconditions must match the observed target; the configured connector customer provides an independent tenant boundary. The provider enforces whether the alias namespace belongs to that customer. Provider-verified secondary domains are supported even if they differ from the connector's sync-selection domain; that filter is not a new alias authorization policy.

Success requires observed attachment in the same pinned account's dedicated alias collection, not merely an accepted insert. `already_present` can be a zero-write same-account replay. A conflicting user, group, or namespace never triggers deletion, transfer, or adoption. Primary addresses and known non-editable aliases are rejected. Provider permission failures and uncertain readbacks remain errors with qualified evidence.
