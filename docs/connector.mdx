...
Group `List` and `Get` both include `group_settings`, using one Groups Settings lookup per returned group. They require the Groups Settings API and `apps.groups.settings` scope. Missing authorization, a failed settings request, or a response with no supported settings fails the group read; a failed list page does not return a partial result. Returned settings contain only values supplied by Google; omitted individual fields remain unknown and are not defaulted. `allow_web_posting` controls the web forum, not permission for external email senders.
...
Enable the Admin SDK API. Add the Cloud Identity API for stable enterprise application IDs. Group sync, targeted group reads, and settings changes also require the Groups Settings API.
...
| Groups Settings API | `groupssettings.googleapis.com` | Required for group reads and settings changes | Group `List`/`Get` and the `modify_group_settings` connector action |
...
<Step>
Repeat for the **Groups Settings API** to enable group sync, targeted reads, and settings changes.
</Step>
</Steps>
...
Google has no read-only Groups Settings scope. Both sets include `apps.groups.settings` because group sync and targeted reads require it; this scope also permits editing group settings. Directory group-read permission alone does not grant access to settings.
