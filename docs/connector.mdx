...
OAuth-token and application-password deletion results retain `deleted_ids`, `already_absent_ids`, `failed_ids`, `skipped_ids`, and `remaining_ids`, together with `inventory_complete` and `observed_at`. A blank skipped ID means an entry could not be identified. If final enumeration fails, an empty `remaining_ids` list is not evidence of absence. Failed actions retain these partial results; consumers must preserve both the error and the result. These are observations at that instant, not proof of global session invalidation. Application grants derived from login activity are not a current credential inventory.
