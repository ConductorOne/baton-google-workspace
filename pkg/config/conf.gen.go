...
func (c *GoogleWorkspace) GetStringSlice(fieldName string) []string {
...
func (c *GoogleWorkspace) GetString(fieldName string) string {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
		return ""
	}
...
func (c *GoogleWorkspace) GetInt(fieldName string) int {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
		return 0
	}
...
func (c *GoogleWorkspace) GetBool(fieldName string) bool {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
		return false
	}
...
func (c *GoogleWorkspace) GetStringMap(fieldName string) map[string]any {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
